...
namespace TheMovie.Services
{
    ...
    public async Task<SearchMovie> SearchMoviesByGenreAsync(string searchTerm, int page)
    {
        var restUrl = $"{baseUrl}discover/movie?api_key={apiKey}&with_genres={searchTerm}&page={page}&language={language}";
    ...
    public async Task<SearchMovie> GetMoviesByCategoryAsync(int page, Enums.MovieCategory category)
        ...
        public async Task<List<Genre>> GetGenreDropDown()
        {
            //HttpClient client = new HttpClient();
            //string response = await client.(urlGenre);
            //List<Genre> genre = JsonConvert.DeserializeObject<List<Genre>>(response);
            //        return genre;
            var restUrl = $"{baseUrl}/genre/movie/list?api_key={apiKey}&language={language}";
