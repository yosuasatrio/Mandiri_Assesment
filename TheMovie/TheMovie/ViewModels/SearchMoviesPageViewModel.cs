...
namespace TheMovie.ViewModels
{
    public class SearchMoviesPageViewModel : BaseViewModel
    {
        ...
        public DelegateCommand SearchByGenreCommand { get; }
        ...
        public SearchMoviesPageViewModel(INavigationService navigationService, IPageDialogService pageDialogService)
        {
            ...
            SearchByGenreCommand = new DelegateCommand(async () => await ExecuteSearchByGenreCommand().ConfigureAwait(false));
        ...
        private async Task ExecuteSearchByGenreCommand()
        {
            if (IsBusy)
                return;
            ...
            try
            {
                SearchResults.Clear();
                currentPage = 1;
                await LoadByGenreAsync(currentPage).ConfigureAwait(true);
            }
            finally
            {
                IsBusy = false;
        ...
        private async Task LoadByGenreAsync(int page)
        {
            try
            {
                genres = genres ?? await ApiService.GetGenresAsync().ConfigureAwait(false);
                var searchMovies = await ApiService.SearchMoviesByGenreAsync(searchTerm, page).ConfigureAwait(false);
                if (searchMovies != null)
                {
                    var movies = new List<Movie>();
                    totalPage = searchMovies.TotalPages;
        ...
        private void PickerGenre_OnSelectedIndexChanged(object sender, EventArgs e) => ExecuteSearchByGenreCommand();
    }
}